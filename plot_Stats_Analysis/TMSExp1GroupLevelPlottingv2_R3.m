...
dataTablePath = ['Z:\StudentFolders\Cody\Projects\TMS\Statistical_Analysis\R3 Paper Stats\Group Level Analysis']; %<- change this if data table is stored somewhere else
...
%% Exp 1 - Mixed Model by Group with Random interaction term & unequal variances
% By group - effect of TMS separated by MEP positive and MEP negative (1X
% STD)
% clc
close all

group1MEPnegPar = [0.9554750 0.35555610;
3.2031376 0.37689333;
2.3782217  0.41906377;
2.1447194 0.41483841;
1.9666008 0.39475939]; % 90% group

group1MEPposPar = [0.9554750 0.35555610;
3.2031376 0.37689333;
3.0261368 0.41543203;
2.4778154 0.43507979;
2.0384374 0.39719218]; % 90% group


group2MEPnegPar = [0.9872704 0.46369882;
2.7275146 0.47285578;
2.6420655 0.54770901
2.3153362 0.53545883
1.7883985 0.53789454];  % 95% group

group2MEPposPar = [0.9872704 0.46369882;
2.7275146 0.47285578;
2.1186308 0.47207908;
2.2267209 0.47197717;
1.7760979 0.47408798];  % 95% group
  
%Plot     
figure('units', 'pixels', 'Position', [-100 -400 1600 1000])
subplot = @(m,n,p) subtightplot(m,n,p,[0.05 0.04], [0.2 0.1], [0.1 0.02]);
offset  = 0.05;
% sgtitle('Norm LLRa by Group', 'FontSize', 20)

subplot(2,2,1)
xVec = (1:5);
plot(xVec, group1MEPnegPar(1:5,1),'md', 'MarkerSize', 16, 'MarkerFaceColor', 'm' ); hold on
p(1) = errorbar(xVec, group1MEPnegPar(1:5,1), 1.96*group1MEPnegPar(1:5,2), '--m', 'LineWidth', 4);
sigline([1 2],[],4,4,'k'); sigline([2 5],[],4.3,4.3,'m');

hold on
plot(xVec, group1MEPposPar(1:5,1),'cd', 'MarkerSize', 16, 'MarkerFaceColor', 'c' ); hold on
p(2) = errorbar(xVec, group1MEPposPar(1:5,1), 1.96*group1MEPposPar(1:5,2), '--c', 'LineWidth', 4);
sigline([2 5],[],4.8,4.8,'c');

set(gca, 'FontSize', 30);
title('90% AMT Group','FontSize',26)
% xticklabels({'Back','Pert','T_1','T_2', 'T_3'})
xticklabels({'','', '', '',''})
xlabel('', 'FontSize', 28)
ylabel({'LLRa [n.u.]'}, 'FontSize', 32, 'FontWeight','bold')
xlim([0.75 5.5])
...
subplot(2,2,2)
xVec = (1:5);
plot(xVec, group2MEPnegPar(1:5,1),'md', 'MarkerSize', 16, 'MarkerFaceColor', 'm' ); hold on
p(1) = errorbar(xVec, group2MEPnegPar(1:5,1), 1.96*group2MEPnegPar(1:5,2), '--m', 'LineWidth', 4);
sigline([1 2],[],4,4,'k');

hold on
plot(xVec, group2MEPposPar(1:5,1),'cd', 'MarkerSize', 16, 'MarkerFaceColor', 'c' ); hold on
p(2) = errorbar(xVec, group2MEPposPar(1:5,1), 1.96*group2MEPposPar(1:5,2), '--c', 'LineWidth', 4);

set(gca, 'FontSize', 30);
title('95% AMT Group','FontSize',26)
% xticklabels({'Back','Pert','T_1','T_2', 'T_3'})
xticklabels({'','', '', '',''})
xlabel('', 'FontSize', 28)
ylabel({''}, 'FontSize', 28, 'FontWeight','bold')
...
legend([p(1);p(2)],{'MEP(-): 1XSTD','MEP(+): 1XSTD'},'FontSize',23,'location','northwest','NumColumns',2)
  

% Figure_path = ['Z:\StudentFolders\Cody\Projects\TMS\Figures\Figs4Paper_R3\'];
% FileName = ['EffectofTMSMode_byGroup_TMSExp1_1XSDThres_MEPSeparated'];
...
% Exp 1 - Mixed Model by Group with Random interaction term & unequal variances
% By group - effect of TMS separated by MEP positive and MEP negative
% (200uV threshold)
% clc
% close all

group1MEPnegPar = [0.9554750	0.37373217;
3.2031376	0.39403659;
2.7147615	0.40327928;
2.4116054	0.41305001;
1.9197815	0.38918113]; % 90% group

group1MEPposPar = [0.9554750	0.37373217;
3.2031376	0.39403659;
2.2822575	0.47223402;
1.8992294	0.45698259;
2.0250110	0.42200506]; % 90% group


group2MEPnegPar = [0.9872704	0.43472204;
2.7275146	0.44448487;
2.3336949	0.45120853;
2.1394607	0.45290523;
1.6946024	0.44820105];  % 95% group

group2MEPposPar = [0.9872704	0.43472204;
2.7275146	0.44448487;
2.1555287	0.52202837;
2.0931902	0.53500774;
1.9927069	0.53370579];  % 95% group
  
%Plot     
% figure('units', 'pixels', 'Position', [-100 -300 1600 800])
% subplot = @(m,n,p) subtightplot(m,n,p,[0.05 0.04], [0.2 0.1], [0.1 0.02]);
offset  = 0.05;
% sgtitle('Norm LLRa by Group', 'FontSize', 20)
...
plot(xVec, group1MEPnegPar(1:5,1),'md', 'MarkerSize', 16, 'MarkerFaceColor', 'm' ); hold on
p(1) = errorbar(xVec, group1MEPnegPar(1:5,1), 1.96*group1MEPnegPar(1:5,2), '--m', 'LineWidth', 4);
sigline([1 2],[],4.4,4.4,'k'); sigline([2 5],[],4,4,'m');

hold on
plot(xVec, group1MEPposPar(1:5,1),'cd', 'MarkerSize', 16, 'MarkerFaceColor', 'c' ); hold on
p(2) = errorbar(xVec, group1MEPposPar(1:5,1), 1.96*group1MEPposPar(1:5,2), '--c', 'LineWidth', 4);
sigline([2 4],[],4.4,4.4,'c'); sigline([2 5],[],4.8,4.8,'c');
...
plot(xVec, group2MEPnegPar(1:5,1),'md', 'MarkerSize', 16, 'MarkerFaceColor', 'm' ); hold on
p(1) = errorbar(xVec, group2MEPnegPar(1:5,1), 1.96*group2MEPnegPar(1:5,2), '--m', 'LineWidth', 4);
sigline([1 2],[],4.3,4.3,'k'); sigline([2 5],[],4,4,'m');
...
legend([p(1);p(2)],{'MEP(-): 200 \muV','MEP(+): 200 \muV'},'FontSize',23,'location','northwest','NumColumns',2)
...
groupMEPnegPar = [0.9713727	0.26939566;
2.9653261	0.28041013;
2.5417610	0.33003849;
2.3032603	0.32473399;
1.9100083	0.31693060]; 

groupMEPposPar = [0.9713727	0.25992840;
2.9653261	0.27132816;
2.5247566	0.28537097;
2.2367982	0.29588760;
1.8984630	0.27832735]; %
...
subplot(1,1,1)
xVec = (1:5);
plot(xVec, groupMEPnegPar(1:5,1),'md', 'MarkerSize', 16, 'MarkerFaceColor', 'm' ); hold on
p(1) = errorbar(xVec, groupMEPnegPar(1:5,1), 1.96*groupMEPnegPar(1:5,2), '--m', 'LineWidth', 4);
sigline([1 2],[],4,4,'k'); sigline([2 5],[],4.3,4.3,'m');

hold on
plot(xVec, groupMEPposPar(1:5,1),'cd', 'MarkerSize', 16, 'MarkerFaceColor', 'c' ); hold on
p(2) = errorbar(xVec, groupMEPposPar(1:5,1), 1.96*groupMEPposPar(1:5,2), '--c', 'LineWidth', 4);
sigline([2 5],[],4.8,4.8,'c'); sigline([2 4],[],5.2,5.2,'c');
...
legend([p(1);p(2)],{'MEP(-): 1XSD','MEP(+): 1XSD'},'FontSize',23,'location','northwest','NumColumns',2)
  

Figure_path = ['Z:\StudentFolders\Cody\Projects\TMS\Figures\Figs4Paper_R3\TMSExp1\'];
FileName = ['TMSExp1_1XSDThres_MEPSeparated_CombinedGroups'];
...
%[0.83;0.87;0.1;0.1]
% Figure_path = ['Z:\StudentFolders\Cody\Projects\TMS\Figures\Figs4Paper_R3\TMSExp1\'];
% FileName = ['TMSExp1_MEPType_ThresholdPlots'];
...
%% Effect of normalized MEP peak on the LLR amplitude (TMS exp 1 group)
...
subplot(1,1,1)
col1 = [46,206,46; 245,120,64; 0,115,179]./255;

p(1) = fplot(@(x) 2.9534492 - 0.0050802*x, [0 , 300],'LineStyle','-','Color',col1(1,:),'LineWidth',4);
hold on
p(2) = fplot(@(x) 2.6430958 - 0.0041134*x, [0 , 300],'LineStyle','-','Color',col1(2,:),'LineWidth',4);
hold on
p(3) = fplot(@(x) 2.1328629 - 0.00356*x, [0 , 300],'LineStyle','-','Color',col1(3,:),'LineWidth',4);

set(gca, 'FontSize', 20);
title('TMS Exp1 Group', 'FontSize', 22);
% xticklabels({'','0','1'})
...
hold on
text(13,3.1,' \bf Y = - 0.005*x + 2.95','FontSize',12,'Rotation',-12)
text(8,2.4,' \bf Y = - 0.004*x + 2.64','FontSize',12,'Rotation',-10)
text(50,1.7,' \bf Y =  - 0.003* + 2.13','FontSize',12,'Rotation',-10)

leg = legend(p(1:3),'T1','T2','T3','location','northeastoutside','FontSize',22);
title(leg,'Latency')


FileName = ['TMSExp1-EffectofNormMEPPeak_vsTMSTiming'];
exportgraphics(gcf,[Figure_path,FileName,'.png'])
